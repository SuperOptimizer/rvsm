...
| 6 — the driver: `run.py`, supervisor, Producer, lookahead, verso gate, rounds, GPU modes | *the driver: the supervisor, the producer, the lookahead, the two gates and the rounds* | done on the CPU: `rvsm/run.py` + `rvsm run` / `status` / `stop` + `tests/test_run_e2e.py`. `ladder()` is NOT in `run.py` (the plan's §5 row put it there): the size ladder is commit 7's `rvsm/sizeladder.py`. Nothing GPU-shaped in it has run on a GPU yet — see `review_checklist.md` §10 |
