...
| `windows_per_region` | `64` | a 1024³ region is 64 windows' worth of volume at 256³. Measured: plain random windows fetch 39.5 B/voxel at a 0.24 hit rate and stall; `--region 1024 --windows-per-region 64` fetches 1.37 B/voxel at 0.98 | `usrm2-streaming.md`; §17 |
...
| `eval_every` | `500` | at 32 Mvox/s the periodic cost (`evaluate` 17-19 s + `val_png` 8.4 s + checkpoint 1.6 s) is 27 s per 500 steps = 54 ms/step amortised, ~4 % of a step. On the A100 at production speed §14 recommends 1000 | §14 |
