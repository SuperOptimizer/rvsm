...
| `fingerprint()` excludes exactly `(steps, eval_every, workers, gpus, rounds, ct_seed, ckpt_act, compile)` and nothing else | `rvsm/config.py:FINGERPRINT_EXCLUDE` | `tests/test_config.py::test_fingerprint_ignores_only_the_resume_fields` |
