...
| `fingerprint()` excludes exactly `(steps, eval_every, workers, gpus, rounds)` and nothing else | `rvsm/config.py:FINGERPRINT_EXCLUDE` | `tests/test_config.py::test_fingerprint_ignores_only_the_resume_fields` |
